...
scene.fog = new THREE.FogExp2(); // 예를 들어, 0.05로 변경
...
const pointlight = new THREE.PointLight (0xFFFFFF, 2); // 밝은 흰색 조명 추가
pointlight.position.set (0, 10, 10); // 조명 위치 조정
group.add (pointlight);

// create the camera
var camera = new THREE.PerspectiveCamera( 20, window.innerWidth/window.innerHeight, 0.1, 1000 );
camera.position.z = 10; // 카메라를 모델에 더 가깝게 이동
...
gltfLoader.load(
    './assets/shiba/scene.gltf', // Replace 'your_model.gltf' with the path to your gltf file
    (gltf) => {
        // Here you can access the loaded gltf object
        const model = gltf.scene;
        
        // Traverse through the model's children to access and adjust material properties
        model.traverse((child) => {
            if (child.isMesh) {
                // Adjust material properties as needed
                child.material.color.set(0xffffff); // Set all colors to white to brighten the model
            }
        });

        // You may need to adjust the position, scale, rotation, or materials of the loaded model as needed
    ...
    (xhr) => {
        // This function will be called while loading is in progress
        console.log((xhr.loaded / xhr.total * 100) + '% loaded');
    },
    (error) => {
        // This function will be called if there is an error loading the model
